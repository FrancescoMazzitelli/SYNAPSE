...
DATASET_NAME = "data_MyDailyTravelData"
...
def _strip_float(s: str) -> str:
    ...
    if s.endswith(".0") and not s.rstrip("0").endswith("."):
        s = s[:-2] if s != "-0.0" else "0"
    return s
...
def load_label_map(dict_path: str = None) -> dict:
    ...
    if dict_path is None:
        dict_path = str(Path(__file__).parent /
                        "configs/Generic/data_MyDailyTravelData/dictionary.csv")
    try:
        import csv
        label_map = {}
...
def _resolve_agent(val, col: str = ""):
    ...
    Steps:
      1. Parse Python‑list format  ['code: text', ...]  → first code
      2. Parse  code: label  → code
      3. Normalise non‑response codes  (-1, -7, -8, -9)  → <NR>
    """
    if pd.isna(val):
        return val
    s = str(val).strip()

    # A) Python list of quoted strings from multi‑select
    ...
    # D) Fallback: look up the label in the survey dictionary
    if col:
        ...
        if lmap:
            ...
            if code:
                return code
            # Substring match: query contained in a dictionary label
...
EXCLUDED_COLS          = {"state", "state_fips", "typday_no", "source_o", "nogowhy_o"}
...
def discover_runs(results_dir: str) -> list[dict]:
    """Scan results/<model>/data_MyDailyTravelData/ for evaluation runs.

    Expected structure:
      results/
        <model>/
          data_MyDailyTravelData/
            oracle/sampled_oracle.csv
            agents/<timestamp>_results.csv
            agents/log.txt
    ...
    for model_dir in sorted(base.iterdir()):
        ...
        # Results may be in agents/ subdirectory or flat in dataset_dir
...
def run_single_evaluation(run_info: dict, out_dir: Path) -> dict:
    """Run the full evaluation pipeline for a single model/data_MyDailyTravelData pair."""
    global CATEGORICAL_MAX_UNIQUE, FREE_TEXT_MIN_UNIQUE, NR_THRESHOLD, EXCLUDED_COLS
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Dynamic LLM Agent Evaluation Suite — data_MyDailyTravelData only")

    ap.add_argument("--results-dir", type=str, default=None,
                    help="Auto-discover runs under this directory (default: results/ if no --results given)")
    ...
    print(f"\n{SEP}")
    print("  LLM TRANSPORTATION AGENT — DYNAMIC EVALUATION SUITE  (data_MyDailyTravelData only)")
    print(SEP)
    ...
    if args.results_dir:
        ...
        if not runs:
            sys.exit(f"[ERROR] No evaluation runs for '{DATASET_NAME}' found under {args.results_dir}/")

        print(f"\n  Discovered {len(runs)} evaluation run(s) for '{DATASET_NAME}':")
        for r in runs:
            print(f"    - {r['model']}")
        ...
        for run_info in runs:
            try:
                ...
                model = run_info["model"]
                print(f"\n  [ERROR] Evaluation failed for {model}/{DATASET_NAME}: {e}")
                all_results.append({
                    "model": model,
                    "dataset": DATASET_NAME,
                    "error": str(e),
        ...
        summary_csv = out_base / "summary_MyDailyTravelData.csv"
        pd.DataFrame(summary_rows).to_csv(summary_csv, index=False)
        print(f"\n{'='*60}")
        print(f"  Global summary saved: {summary_csv}")
        print(f"{'='*60}")

        # Console comparison table
        print(f"\n{'='*60}")
        print("  GLOBAL COMPARISON  (data_MyDailyTravelData)")
        print(f"{'='*60}")
